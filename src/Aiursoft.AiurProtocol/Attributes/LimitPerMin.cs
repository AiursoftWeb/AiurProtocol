    ...
    public override void OnActionExecuting(ActionExecutingContext context)
    {
        ...
        if (tempDictionary.ContainsKey(ip + path))
        {
            ...
            if (tempDictionary[ip + path] > _limit)
            {
                ...
                if (ReturnJson)
                {
                    context.Result = (context.Controller as ControllerBase)
                        ?.Protocol(
                            errorType: ErrorType.TooManyRequests, 
                            errorMessage: "You are requesting our API too frequently and your IP is blocked.") 
                        ?? throw new InvalidOperationException("HttpContext.Controller is not a controller base!");
                }
                else
                {
